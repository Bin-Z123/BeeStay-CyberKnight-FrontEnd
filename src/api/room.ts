...
export const RoomAPI = defineStore("room", () => {
  ...
  const getAllRooms = async (): Promise<ResponseRoom> => {
    ...
    try {
      const response = await axios.get<ResponseRoom>(`${baseUrl}/rooms`, {
  ...
  const createRoom = async (
    ...
    try {
      ...
      const roomBlob = new Blob([JSON.stringify(room)], {
  ...
  const updateRoom = async (
    ...
    try {
      ...
      const response = await axios.put<RoomResponse>(
        `${baseUrl}/mana/rooms/${room.id}`,
        formData,
        { withCredentials: true }
      );
      toast.success("Thông báo", {
        description:
          "Cập nhật phòng " + response.data.roomNumber + " thành công!",
      });
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  const getRoomById = async (id: number): Promise<ResponseRoom> => {
    isLoading.value = true;
    try {
      const response = await axios.get<ResponseRoom>(`${baseUrl}/rooms/${id}`, {
        withCredentials: true,
      });
...
const getRoomProfile = (roomId: Ref<number | undefined>) => {
  return myAxios.get(`/rooms/${roomId.value}`);
};

export { getRoomsList, getRoomProfile };
