...
export const Facilities = defineStore("facilities", () => {
  ...
  const updateFacility = async (
    ...
    try {
      ...
      formData.append("facility", faciBlob);
      formData.append("file", file);
      const response = await axios.put<Facility>(
        `${baseUrl}/mana/facility/${facility.id}`,
        formData,
        {
          withCredentials: true,
        }
  ...
  const deleteFacility = async (facilityId: number): Promise<void> => {
    isLoading.value = true;
    try {
      await axios.delete(`${baseUrl}/mana/facility/${facilityId}`, {
        withCredentials: true,
      });
