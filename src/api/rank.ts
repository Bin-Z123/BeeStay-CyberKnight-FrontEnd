...
export const Rank = () => {
  ...
  const updateRank = async (rank: Rank): Promise<RankResponse> => {
    isLoading.value = true;
    try {
      const response = await axios.put<RankResponse>(
        `${baseUrl}/mana/rank/${rank.id}`,
        rank
      );
      if (response.status === 200) {
        toast.success("Thông báo", {
  ...
  const deleteRank = async (id: number): Promise<RankResponse> => {
    isLoading.value = true;
    try {
      const response = await axios.delete<RankResponse>(
        `${baseUrl}/mana/rank/${id}`
      );
      if (response.status === 200) {
        toast.success("Thông báo", {
