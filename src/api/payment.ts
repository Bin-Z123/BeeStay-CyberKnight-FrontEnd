...
export const PaymentAPI = defineStore("payment", () => {
  ...
  const createPaymentPayOsLink = async (
    request: CreatePaymentLinkRequest
  ): Promise<PaymentPayOsResponse> => {
    isLoading.value = true;
    try {
      // console.log("Creating Payment Link with Request: ", JSON.stringify(request, null, 2));
      const response = await axios.post<PaymentPayOsResponse>(
        `${baseUrl}/orderPayOS/create`,
        request,
        {
  ...
  const getPaymentOsData = async (
    bookingId: number
  ): Promise<PaymentPayOsResponse> => {
    isLoading.value = true;
    try {
      const response = await axios.get<PaymentPayOsResponse>(
        `${baseUrl}/orderPayOS/${bookingId}`,
        {
          withCredentials: true,
        }
      );
  ...
  const createPaymentByCash = async (request: PaymentCashRequest) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        toast.error(
          error.response?.data.message || "Failed to create cash payment",
          {
            action: {
              label: "Thoát",
            },
  ...
  const getPaymentPaidByBookingId = async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        toast.error(
          error.response?.data.message || "Failed to calculate payment",
          {
            action: {
              label: "Thoát",
            },
