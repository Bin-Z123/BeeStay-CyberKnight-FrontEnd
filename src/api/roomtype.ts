...
import {
  MaybeRef,
  MaybeRefOrGetter,
} from "@tanstack/vue-query/build/legacy/types";
...
export const RoomType = defineStore("roomtype", () => {
  ...
  const getAllRoomType = async (): Promise<RoomTypeResponse> => {
    try {
      const response = await axios.get<RoomTypeResponse>(
        `${baseUrl}/roomTypes`,
        {
  ...
  const createRoomType = async (roomtype: RoomType): Promise<RoomType> => {
    ...
    try {
      const response = await axios.post<RoomType>(
        `${baseUrl}/mana/roomTypes`,
        roomtype,
        {
          withCredentials: true,
        }
  ...
  const updateRoomType = async (roomtype: RoomType): Promise<RoomType> => {
    isLoading.value = true;
    try {
      const response = await axios.put<RoomType>(
        `${baseUrl}/mana/roomTypes/${roomtype.id}`,
        roomtype,
        {
          withCredentials: true,
        }
      ...
      toast.error("Thông báo", {
        description:
          "Lỗi cập nhật loại phòng: " +
          (error.response?.data?.message || error.message),
      });
      throw error;
    } finally {
  ...
  const deleteRoomType = async (roomtypeId: number): Promise<void> => {
    isLoading.value = true;
    try {
      await axios.delete(`${baseUrl}/mana/roomTypes/${roomtypeId}`, {
        withCredentials: true,
      });
      toast.success("Thông báo", {
  ...
  const getRoomTypeById = async (roomtypeId: number): Promise<RoomType> => {
    try {
      const response = await axios.get<{ code: number; data: RoomType }>(
        `${baseUrl}/roomTypes/${roomtypeId}`,
        { withCredentials: true }
      );
...
const getRoomTypesList = async () => {
  return myAxios.get("/roomTypes");
};
const getRoomTypeById = async (roomtypeId: Ref<number | undefined>) => {
  return myAxios.get(`/roomTypes/${roomtypeId.value}`);
};

export { getRoomTypesList, getRoomTypeById };
