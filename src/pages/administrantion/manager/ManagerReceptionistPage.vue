<template>
        ...
        <tbody class="text-gray-700">
          ...
          >
            ...
            <td class="py-2 flex justify-center items-center gap-2 h-full">
              <button
                @click="handleUpdateBlock(user)"
                v-if="user.eblacklist === 2"
              ...
              <button
                @click="handleUpdateBlock(user)"
                v-else
                class="hover:text-green-700 m-1.5 text-green-500"
              >
                <LockKeyholeOpen class="w-5.5 h-5.5" />
              </button>

              <button
                @click="openUpdateReceptionist(user)"
  ...
  <CreateReceptionistDialog
    v-model:open="openCreateUserDialog"
  ></CreateReceptionistDialog>
...
const filteredUsers = computed(() => {
  return Users.users.filter((user) => {
    const matchRole = [2, 3, 4].includes(user.role?.id);
    const matchSearch =
