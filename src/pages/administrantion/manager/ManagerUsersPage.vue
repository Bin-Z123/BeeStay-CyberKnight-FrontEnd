<template>
  <section>
    <Tabs default-value="user" class="">
      <div class="flex">
        <div class="w-1/2">
          <TabsList
            class="grid grid-cols-3 mx-4 mt-2 bg-muesli-300 rounded-b-none shadow-sm"
          >
            <TabsTrigger
              value="user"
          ...
          >
            ...
            <tbody class="text-gray-700">
              ...
              >
                ...
                <td class="py-2" :class="getBlacklistClass(user.eblacklist)">
                  {{ user.eblacklist == 0 ? "Normal" : "Blacklist" }}
                </td>
                <td class="py-2">
                  {{
                    user.rank && user.rank.nameRank
                      ? user.rank.nameRank
                      : "Không có"
                  }}
                </td>
                ...
                <td class="py-2 flex justify-center items-center gap-5 h-full">
                  <button
                    @click.prevent="handleUpdateEblacklist(user)"
                  ...
                  <button
                    @click.prevent="openUpdatePassword(user)"
                    class="text-red-500"
                  >
        ...
        <div class="px-4">
          ...
          >
            ...
            <tbody class="text-gray-700">
              <tr
                class="hover:bg-muesli-100 transition odd:bg-white even:bg-gray-100"
                v-for="blacklist in filteredBlackList"
                :key="blacklist.id"
              >
                <td class="py-2">{{ blacklist.fullname }}</td>
                <td class="py-2">
                  {{ blacklist.gender == true ? "Nam" : "Nữ" }}
                </td>
                <td class="py-2">{{ blacklist.birthday }}</td>
                <td class="py-2">{{ blacklist.email }}</td>
                <td
                  class="py-2"
                  :class="getBlacklistClass(blacklist.eblacklist)"
                >
                  {{ blacklist.eblacklist == 0 ? "Normal" : "Blacklist" }}
                </td>
                <td class="py-2">
                  {{
                    blacklist.rank && blacklist.rank.nameRank
                      ? blacklist.rank.nameRank
                      : "Không có"
                  }}
                </td>
                ...
                <td class="py-2 flex justify-center items-center gap-5 h-full">
                  <button
                    @click.prevent="handleUpdateEblacklistAgain(blacklist)"
                    class="hover:text-red-700 m-1.5 text-red-500"
                  >
                    <LockKeyhole class="w-5.5 h-5.5" />
                  </button>
                  <button
                    @click.prevent="openUpdateUser(blacklist)"
                    class="text-blue-400 hover:text-blue-700"
                  >
                    <SquarePen class="w-5.5 h-5.5" />
                  </button>
  ...
  <UpdateCustomerDialog
    v-model:open="openUpdateUserDialog"
    :user="selectedUser"
  ></UpdateCustomerDialog>
  <UpdatePasswordUser
...
const filteredGuests = computed(() =>
  Guests.guest.filter(
    (guest) =>
      guest.fullname.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
