<template>
  <section class="">
    <div class="flex">
      <div class="w-2/3">
        <div class="flex gap-2 items-center">
          ...
          >
            <template #day="{ date, day }">
              <div
                :class="
                  isToday(date)
                    ? 'bg-green-500 text-white w-full h-full rounded-2xl'
                    : ''
                "
              >
                {{ day }}
          ...
          <Button
            @click="selectToday"
            class="me-4 h-10 bg-white text-muesli-400 border border-muesli-400 hover:bg-muesli-400 hover:text-white my-3"
            >Hôm nay</Button
          >
    ...
    <div
      v-if="isFetchingBookings"
      class="flex justify-center items-center h-200 w-full space-x-1"
    >
    ...
    <div v-else class="shadow-lg px-4 pb-4 h-[622px]">
      <table class="w-full border border-gray-300 text-sm text-center bg-white">
        ...
        <tbody class="text-gray-700">
          <tr
            class="hover:bg-muesli-100 focus:bg-muesli-200 transition odd:bg-white even:bg-gray-100"
            tabindex="0"
            v-for="booking in paginatedRoomTypes"
            :key="booking.id"
          ...
          >
            ...
            <td class="py-2">
              {{
                ...
                booking.guestBooking.fullname + " (Guest)"
              }}
            </td>
            <td class="py-2">
              {{ booking.user?.phone || booking.guestBooking?.phone }}
            </td>
            <td class="py-2">
              {{ booking.user?.email || booking.guestBooking?.email }}
            </td>
            <td class="py-2">
              {{ formatDateWithTimeToUI(booking?.checkInDate) }}
            </td>
            <td class="py-2">
              {{ formatDateWithTimeToUI(booking?.checkOutDate) }}
            </td>
    ...
    <!-- Menu Context -->
    <div
      v-if="menu.isOpen && authStore.user?.role?.roleName !== 'ADMIN'"
      class="absolute bg-white border rounded-xs shadow-md z-50 w-48"
      :style="{ top: menu.y + 'px', left: menu.x + 'px' }"
    >
      <p class="bg-muesli-100/50 px-4 py-2 font-bold text-muesli-400">
        Khách:
        {{ menu.data.guestBooking?.fullname || menu.data.user?.fullname }}
      </p>
      <div v-if="menu.data.bookingStatus == 'CONFIRMED'">
        <button
          @click="isOpenConfirmBooking = true"
        ...
        <hr />
        <button
          @click="() => (isOpenAlertCancelBooking = true)"
          class="w-full text-left px-4 py-2 hover:bg-gray-100"
        >
          Hủy phòng
        </button>
      ...
      <div v-if="menu.data.bookingStatus == 'CANCEL'">
        <button
          class="w-full text-left px-4 py-2 hover:bg-gray-100"
          @click="openTabBooking(menu.data.id)"
        >
          Xem Chi Tiết
        </button>
      </div>
  ...
  <AsyncAlertCancelBooking
    v-model:open="isOpenAlertCancelBooking"
    :Booking="menu.data.id"
  ></AsyncAlertCancelBooking>
  <AsyncConfirmBookingDialog
    v-if="isOpenConfirmBooking"
    v-model:open="isOpenConfirmBooking"
    :Booking="menu.data"
  ...
  <AsyncPendingBookingDialog
    v-model:open="isOpenPendingBooking"
  ></AsyncPendingBookingDialog>
  <AsyncAlertDialog
    v-model:open="openAlert"
    :IdBooking="menu.data.id"
  ></AsyncAlertDialog>
</template>
...
const { data: bookingsList, isFetching: isFetchingBookings } =
  useBookingsList();
const router = useRouter();
const AsyncConfirmBookingDialog = defineAsyncComponent({
  loader: () =>
    import(
      "@/components/administration/BookingDialog/ConfirmBookingDialog.vue"
    ),
});
const AsyncAlertDialog = defineAsyncComponent({
  loader: () =>
    import("@/components/administration/BookingDialog/AlertDialog.vue"),
});
const AsyncPendingBookingDialog = defineAsyncComponent({
  loader: () =>
    import(
      "@/components/administration/BookingDialog/PendingBookingDialog.vue"
    ),
});
const AsyncAlertCancelBooking = defineAsyncComponent({
  loader: () =>
    import("@/components/administration/BookingDialog/AlertCancelBooking.vue"),
});
const AsyncNewBookingDialog = defineAsyncComponent({
  loader: () =>
    import("@/components/administration/BookingDialog/NewBookingDialog.vue"),
...
const fillterselectedStatus = computed(() => {
  ...
  return filteredBookings.value.filter(
    (b) => b.bookingStatus === selectedStatus.value
  );
});
