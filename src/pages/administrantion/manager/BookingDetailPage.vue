<template>
  ...
  <div
    v-if="isLoading"
    class="flex justify-center items-center h-200 w-full space-x-2"
  >
  ...
  <div v-else class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 printable-area">
    <div class="bg-white rounded-lg overflow-hidden">
      <div class="p-6 sm:p-8">
        ...
        <div
          class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b"
        >
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
              ...
              <span class="font-semibold text-gray-700"
                >#BK-{{ BookingStore.bookingTicket?.id }}</span
              >
            </p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 py-6 border-b">
          <div>
            ...
            <p class="text-lg font-semibold text-gray-800">
              {{
                formatDateWithTimeToTicket(
                  BookingStore.bookingTicket?.checkInDate ?? new Date()
                )
              }}
            </p>
          ...
          <div>
            ...
            <p class="text-lg font-semibold text-gray-800">
              {{
                formatDateWithTimeToTicket(
                  BookingStore.bookingTicket?.checkOutDate ?? new Date()
                )
              }}
            </p>
            <p class="text-sm text-gray-500">
          ...
          <div>
            ...
            <p class="text-lg font-semibold text-gray-800">
              {{ numberOfNights }} đêm,
              {{
                quantityGuest == 0 ? "Chưa xếp phòng" : quantityGuest + " khách"
              }}
            </p>
          </div>
        </div>
        ...
        <div class="py-6 border-b">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            <div>
              ...
              {{
                BookingStore.bookingTicket?.user?.fullname ||
                BookingStore.bookingTicket?.guestBooking.fullname
              }}
            </div>
            <div>
              <span class="font-semibold">Hạng thành viên:</span>
              <span v-if="BookingStore.bookingTicket?.user">{{
            ...
            <div>
              ...
              {{
                BookingStore.bookingTicket?.user?.email ||
                BookingStore.bookingTicket?.guestBooking.email
              }}
            </div>
            <div>
              <span class="font-semibold">Số điện thoại:</span>
              {{
                BookingStore.bookingTicket?.user?.phone ||
                BookingStore.bookingTicket?.guestBooking.phone
              }}
            </div>
            <div>
              <span class="font-semibold">CCCD/CMND:</span>
              {{
                BookingStore.bookingTicket?.user?.cccd ||
                BookingStore.bookingTicket?.guestBooking.cccd
              }}
            </div>
          </div>
        </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              ...
              <tbody>
                <tr
                  class="border-b"
                  v-for="(bkdt, index) in BookingStore.bookingTicket
                    ?.bookingDetails"
                  :key="index"
                >
                  <td class="p-3">
                    ...
                    <p class="text-sm text-gray-500">
                      {{ bkdt.roomType.size }}m², giường King, tối đa
                      {{ bkdt.roomType.peopleAbout }} người
                    </p>
                  </td>
                  <td class="p-3 text-center">{{ bkdt.quantity }}</td>
                  <td class="p-3 text-right">
                    {{ formatVND(bkdt.roomType.price) }}
                  </td>
                  <td class="p-3 text-right">
                    {{
                      formatVND(
                        calculatePriceForRoomType(
                          bkdt.roomType.price,
                          bkdt.quantity
                        )
                      )
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              ...
              <tbody>
                <tr
                  class="border-b"
                  v-for="(service, index) in BookingStore.bookingTicket
                    ?.bookingFacilities"
                  :key="index"
                >
                  <td class="p-3">
                    <p class="font-semibold text-gray-800">
                      {{ service.facilities[0].facilityName }}
                    </p>
                  </td>
                  <td class="p-3 text-center">
                    {{ formatVND(service.facilities[0].price) ?? 0 }}
                  </td>
                  <td class="p-3 text-right">
                    {{ formatVND(service.facilities[0].price) ?? 0 }}
                  </td>
        ...
        <div class="py-6 border-b">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
              ...
              v-for="(stay, index) in staysWithNightDetails"
              :key="index"
            >
              <h4 class="font-semibold text-gray-700">
                {{ getRoomByIdForStay(stay.roomId) }} -
                {{ stay.numberOfNights }} Đêm
              </h4>
              <p class="text-sm text-gray-600">
                ...
                <span
                  >{{
                    formatDateWithTimeToHour(
                      new Date(stay?.actualCheckIn) ?? new Date()
                    )
                  }}
                </span>
                <span>{{
                  formatDateWithTimeToTicket(new Date(stay?.actualCheckIn))
                }}</span>
              </p>
              <p class="text-sm text-gray-600">
                Ngày trả phòng:
                <span
                  >{{
                    formatDateWithTimeToHour(
                      new Date(stay?.actualCheckOut) ?? new Date()
                    )
                  }}
                </span>
                <span>{{
                  formatDateWithTimeToTicket(new Date(stay?.actualCheckOut))
                }}</span>
              </p>
              <ul
                class="mt-2 text-sm text-gray-600 list-disc list-inside"
                v-for="guest in stay.infoGuests"
                :key="guest.id"
              >
        ...
        <div
          v-if="BookingStore.bookingTicket?.bookingStatus === 'CANCEL'"
          class="outline outline-red-200 p-2 rounded-xl bg-red-100/30"
        >
          <div class="text-center my-2 font-bold text-red-600">
            Đơn đặt phòng đã hủy
          </div>
          <div
            class="mx-auto bg-red-100 rounded-full w-24 h-24 flex items-center justify-center"
          >
            <svg
            ...
            <div class="space-y-3">
              <div class="flex justify-between">
                <span class="text-gray-600"
                  >Tiền phòng ({{ quantityRoom }} phòng x {{ night }} đêm)</span
                >
                <span class="font-medium text-gray-800">{{
                  formatVND(totalAmountRoom)
                }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Tiền dịch vụ</span>
                <span class="font-medium text-gray-800">{{
                  formatVND(totalPriceFacility)
                }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Tạm tính</span>
                <span class="font-medium text-gray-800">{{
                  formatVND(totalAmountRoom + totalPriceFacility)
                }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600"
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-800">{{
                  formatVND(paymentPaid ?? 0)
                }}</span>
              </div>
              <div
                class="flex justify-between text-lg font-bold bg-yellow-100 p-3 rounded-lg"
              >
                <span class="text-yellow-800 my-auto w-48"
                  >Cần thanh toán:
                </span>
              ...
              >
                <h4 class="font-bold text-gray-700 text-center mb-4">
                  Ghi nhận thanh toán
                </h4>
                ...
                <fieldset class="flex justify-center gap-4">
                  ...
                  <div>
                    ...
                    <label
                      for="pay-cash"
                      class="block cursor-pointer rounded-lg border border-gray-300 p-2 text-sm font-medium shadow-sm peer-checked:border-blue-500 peer-checked:ring-1 peer-checked:ring-blue-500"
                ...
                >
                  <label
                    ...
                    >Số tiền khách đưa (VND)</label
                  >
                  <input
                    type="number"
                    id="cash-input"
                    v-model="paymentCash.amount"
                  ...
                  <p class="text-xs text-red-500">
                    * Lưu ý: Nhập tối đa
                    {{ formatVND(paymentOsdata?.amount ?? 0) }}
                  </p>
                  <button
                  ...
                  >
                    ...
                    />
                    Xác nhận thanh toán
                    <span class="ml-1" v-if="paymentCash.amount > 0">{{
                      formatVND(paymentCash.amount)
                    }}</span>
                  </button>
                </div>
                ...
                >
                  <p class="text-sm text-gray-600 mb-3">
                    Nhấn nút bên dưới để hiển thị mã QR cho khách hàng quét.
                  </p>
                  <button
                    id="show-qr-button"
                ...
                <p
                  ...
                  class="hidden mt-3 text-center text-sm font-semibold text-green-600"
                >
      ...
      <div class="bg-gray-50 p-6 text-center no-print">
        ...
        <p class="text-xs text-gray-500 mt-4">
          Cảm ơn bạn đã lựa chọn dịch vụ của chúng tôi. Vui lòng liên hệ bộ phận
          hỗ trợ nếu có bất kỳ thắc mắc nào.
        </p>
      </div>
    </div>
    ...
    <transition name="fade">
      ...
      >
        ...
        >
          <!-- Header -->
          <div
            class="bg-white p-5 rounded-t-2xl border-b border-gray-200 flex justify-between items-center"
          >
            <div class="flex items-center space-x-3">
              <img
                src="../../../assets/images/LogoBeeStay2.svg"
                alt="VietQR Logo"
                class="h-8"
              />
          ...
          <div class="p-6">
            <p class="text-sm text-gray-600 mb-4">
              Quét mã dưới đây để thanh toán từ <br />
              <strong>ứng dụng Ngân hàng</strong> hoặc
              <strong>Ví điện tử</strong> của bạn.
            </p>
            <div
              class="flex justify-center bg-white p-4 rounded-xl shadow-inner"
            >
          ...
          <div class="px-6 pb-6 space-y-4">
            <div
              class="text-left bg-white p-4 rounded-xl border border-gray-200"
            >
              <div
              ...
              >
                ...
                <span id="qr-amount" class="font-bold text-lg text-blue-600">{{
                  formatVND(paymentOsdata.amount)
                }}</span>
              </div>
              <div
                class="flex justify-between items-center py-2 border-b border-gray-100"
              ...
              >
                ...
                <div class="flex items-center space-x-2">
                  <span
                    ...
                    class="font-semibold text-gray-800"
                    >{{
                      paymentStore.paymentOsData.data.accountNumber ?? 0
                    }}</span
                  >
                  <button
                    class="copy-btn p-1 text-gray-500 hover:text-blue-600"
              ...
              <div class="flex justify-between items-center pt-2">
                ...
                <div class="flex items-center space-x-2">
                  <span
                    ...
                    class="font-semibold text-gray-800"
                    >{{
                      paymentStore.paymentOsData.data.description ?? 0
                    }}</span
                  >
                  <button
                    class="copy-btn p-1 text-gray-500 hover:text-blue-600"
          ...
          <div class="bg-white p-4 rounded-b-2xl border-t border-gray-200">
            <p class="text-xs text-gray-500">
              Đơn hàng <strong id="qr-order-code"></strong> sẽ hết hạn sau ít
              phút.
            </p>
            <p>
              *Nếu không quét mã được hãy bấm vào <br /><a
                class="text-blue-600 hover:underline"
                @click="handleOpenVietQR"
                >Quét mã tại đây</a
              >
            </p>
          </div>
...
onMounted(async () => {
  ...
  try {
    ...
    console.log(
      "Booking Detail Page Mounted with ID:",
      JSON.stringify(BookingStore.bookingTicket, null, 2)
    );
  ...
  if (BookingStore.bookingTicket?.bookingDetails) {
    BookingStore.bookingTicket.bookingDetails.forEach((detail) => {
      if (detail.roomType && detail.quantity) {
        totalPriceRoom.value += calculatePriceForRoomType(
          detail.roomType.price,
          detail.quantity
        );
      }
    });
...
watch(
  () => paymentStore.paymentOsData.data,
  (newValue) => {
    if (newValue) {
...
const night = computed(() => {
  return staysWithNightDetails.value.reduce(
    (total, stay) => stay.numberOfNights,
    0
  );
});
</script>
