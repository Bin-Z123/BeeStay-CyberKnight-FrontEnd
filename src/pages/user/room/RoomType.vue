<template>
  <section>
    <div class="bg-white">
      <div class="w-full h-[700px] relative">
        <div
          class="bg-[url('@/assets/images/header__bg.webp')] bg-cover bg-center h-full w-full"
        ></div>
      ...
      <div class="relative z-10">
        ...
        >
          ...
          <form
            action=""
            class="bg-white rounded-2xl shadow-xl -mt-130 lg:-mt-80 text-[20px]"
          >
            <div
            ...
            >
              ...
              <div
                class="flex items-center justify-end border-gray-400 lg:w-2/14"
              >
                <button
    ...
    <div class="bg-gray-50">
      <div class="py-15 container mx-auto flex lg:flex-row flex-col gap-5">
        <div class="flex flex-col lg:w-2/3 px-4 lg:px-0">
          <div
            v-for="roomType in roomsFullMatched"
          ...
          <div v-if="roomsWithAvailable.length" class="flex flex-col gap-10">
            <div
              v-for="(roomType, index) in roomsWithAvailable"
              :key="roomType.roomTypeId"
              :class="[
            ...
            >
              <div class="lg:w-2/5 px-4 lg:px-0">
                <img
                  v-if="roomType.availableRoomDTO?.[0]?.roomImage?.[0]?.url"
                  :src="
                    getImageUrl(roomType.availableRoomDTO[0].roomImage[0].url)
                  "
                  alt="Ảnh phòng"
                  :class="[
                    'w-full h-65 object-cover',
                    index % 2 === 0 ? 'lg:rounded-l-2xl' : 'lg:rounded-r-2xl',
                  ]"
                />
              </div>
              <div class="lg:w-3/5 flex flex-col gap-5 justify-center px-10">
                ...
                <div class="flex items-center gap-4 text-gray-500">
                  <span class="flex items-center gap-1">
                    <Square class="inline-block w-5 h-5" />{{ roomType.size }}
                    m²
                  </span>
                  <span class="flex items-center gap-1">
                    <User class="inline-block w-5 h-5" />{{
                      roomType.peopleAbout
                    }}
                    người
                  </span>
                </div>
                <p class="text-xl font-bold">
                  ...
                  }}<span class="text-gray-500 font-normal">/ngày</span>
                </p>
                <div class="flex">
                  <div class="w-1/2">
                    <a
                      :href="getRoomDetails(roomType.roomTypeId)"
                      target="_blank"
                  ...
                  <div class="w-1/2 flex justify-end items-center">
                    <div
                      class="flex items-center gap-2 border border-gray-300 rounded-lg px-2 py-1"
                    >
                      <button
                        @click="decrementRoom(roomType)"
                        :disabled="!numberOfNights"
                        class="px-2 font-bold text-lg disabled:text-gray-300 disabled:cursor-not-allowed"
                      >
                        -
                      </button>
                      <input
                        type="text"
                        :value="
                          selectedRooms[roomType.roomTypeId]?.quantity || 0
                        "
                        class="w-12 text-center border-x"
                        readonly
                      />
                      <button
                        @click="incrementRoom(roomType)"
                        :disabled="!numberOfNights"
                        class="px-2 font-bold text-lg disabled:text-gray-300 disabled:cursor-not-allowed"
                      >
                        +
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-4">
              ...
              <div class="flex flex-col gap-2">
                <p class="text-gray-500">
                  Ngày Đến:
                  <span class="font-bold text-black float-end">{{
                    formatDate(checkin)
                  }}</span>
                </p>
                <p class="text-gray-500">
                ...
                <p class="font-bold">
                  Tổng Số Đêm:
                  <span class="font-bold float-end">{{
                    numberOfNights || 0
                  }}</span>
                </p>
              </div>
              ...
              <div class="flex flex-col gap-2 min-h-[50px]">
                ...
                <div v-else-if="bookingSummary.length">
                  <div v-for="item in bookingSummary" :key="item.roomTypeId">
                    <p class="text-gray-500">
                      {{ item.nameRoomType }} x {{ item.quantity }}:
                      <span class="font-bold text-black float-end">{{
              ...
              <hr />
              <div class="flex flex-col gap-3 h-[250px] overflow-y-auto">
                <div
                ...
                >
                  <div>
                    ...
                    <p class="text-xs text-gray-500">
                      {{
                        formatCurrency(facility.price) == 0
                          ? "Miễn Phí"
                  ...
                  >
                    <button
                      @click.prevent="decrementFacility(facility)"
                      class="px-2 font-bold text-lg disabled:text-gray-300 disabled:cursor-not-allowed"
                    >
                      -
                    </button>
                    <input
                      type="text"
                      :value="selectedFacilityQuantities[facility.id] || 0"
                      class="w-10 text-center bg-transparent"
                      readonly
                    />
                    <button
                      @click.prevent="incrementFacility(facility)"
              ...
              <div class="flex justify-between items-center">
                ...
                <p class="text-xl font-bold text-muesli-400">
                  {{ formatVND(totalPrice) }}
                </p>
              </div>
              <button
                @click.prevent="proceedToBooking()"
  ...
  <section>
    ...
    <BookingUserDialog
      v-model:open="isBookingUserDialogOpen"
    ></BookingUserDialog>
  </section>
</template>

<script setup>
import { Square, User } from "lucide-vue-next";
...
const totalFacilitiesPrice = computed(() => {
  ...
  for (const facilityId in selectedFacilityQuantities.value) {
    ...
    const facility = allAvailableFacilities.find(
      (f) => f.id === Number(facilityId)
    );
...
const totalPrice = computed(() => {
  return (
    bookingSummary.value.reduce((total, item) => total + item.subtotal, 0) +
    totalFacilitiesPrice.value
  );
});
const proceedToBooking = () => {
  ...
  bookings.selectedFacilities = Object.entries(selectedFacilityQuantities.value)
    .map(([id, quantity]) => {
      const facilityDetails = allAvailableFacilities.find(
        (f) => f.id === Number(id)
      );
      if (facilityDetails) {
        return {
