      ...
      <Tabs default-value="upcoming" class="w-full">
        ...
        <TabsContent value="upcoming">
            <div v-if="upcomingBookings.length > 0">
                <div v-for="booking in paginatedUpcomingBookings" :key="booking.id" class="border rounded-lg p-4 mb-4 shadow-md bg-white hover:shadow-lg transition-shadow duration-300">
                    ...
                    <div class="flex flex-col md:flex-row gap-5 mt-4">
                        <div class="w-full md:w-1/3"><img :src="filterRoomTypeImage(booking.bookingDetails[0].roomType.id)" alt="Room Image" class="w-full h-48 object-cover rounded-md"></div>
                ...
                <div v-if="totalUpcomingPages > 1" class="flex justify-center items-center gap-4 mt-6">
                    <button @click="upcomingCurrentPage--" :disabled="upcomingCurrentPage === 1" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Trước</button>
                    <span class="text-sm text-gray-700">Trang {{ upcomingCurrentPage }} / {{ totalUpcomingPages }}</span>
                    <button @click="upcomingCurrentPage++" :disabled="upcomingCurrentPage === totalUpcomingPages" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Sau</button>
                </div>
            </div>
        ...
        <TabsContent value="ongoing">
             <div v-if="ongoingBookings.length > 0">
                 <div v-for="booking in paginatedOngoingBookings" :key="booking.id" class="border rounded-lg p-4 mb-4 shadow-md bg-white">
                    <div class="flex justify-between items-center pb-3 border-b">
                        <p class="text-sm text-gray-600">Mã Booking: <span class="font-bold text-black">#{{ booking.id }}</span></p>
                        <span :class="['font-semibold text-sm px-3 py-1 rounded-full', getStatusClass(booking.bookingStatus)]">{{ getStatusText(booking.bookingStatus) }}</span>
                    </div>
                 ...
                 <div v-if="totalOngoingPages > 1" class="flex justify-center items-center gap-4 mt-6">
                    <button @click="ongoingCurrentPage--" :disabled="ongoingCurrentPage === 1" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Trước</button>
                    <span class="text-sm text-gray-700">Trang {{ ongoingCurrentPage }} / {{ totalOngoingPages }}</span>
                    <button @click="ongoingCurrentPage++" :disabled="ongoingCurrentPage === totalOngoingPages" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Sau</button>
                </div>
            </div>
        ...
        <TabsContent value="history">
            <div v-if="pastBookings.length > 0">
                <div v-for="booking in paginatedPastBookings" :key="booking.id" class="border rounded-lg p-4 mb-4 shadow-md bg-white">
                     <div class="flex justify-between items-center pb-3 border-b">
                         <p class="text-sm text-gray-600">Mã Booking: <span class="font-bold text-black">#{{ booking.id }}</span></p>
                 ...
                 <div v-if="totalPastPages > 1" class="flex justify-center items-center gap-4 mt-6">
                    <button @click="pastCurrentPage--" :disabled="pastCurrentPage === 1" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Trước</button>
                    <span class="text-sm text-gray-700">Trang {{ pastCurrentPage }} / {{ totalPastPages }}</span>
                    <button @click="pastCurrentPage++" :disabled="pastCurrentPage === totalPastPages" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Sau</button>
                </div>
            </div>
        ...
        <TabsContent value="cancelled">
            <div v-if="cancelledBookings.length > 0">
                 <div v-for="booking in paginatedCancelledBookings" :key="booking.id" class="border rounded-lg p-4 mb-4 shadow-md bg-gray-50">
                     <div class="flex justify-between items-center pb-3 border-b">
                ...
                <div v-if="totalCancelledPages > 1" class="flex justify-center items-center gap-4 mt-6">
                    <button @click="cancelledCurrentPage--" :disabled="cancelledCurrentPage === 1" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Trước</button>
                    <span class="text-sm text-gray-700">Trang {{ cancelledCurrentPage }} / {{ totalCancelledPages }}</span>
                    <button @click="cancelledCurrentPage++" :disabled="cancelledCurrentPage === totalCancelledPages" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Sau</button>
                </div>
            </div>
...
const totalUpcomingPages = computed(() => Math.ceil(upcomingBookings.value.length / itemsPerPage));
const totalOngoingPages = computed(() => Math.ceil(ongoingBookings.value.length / itemsPerPage));
const totalPastPages = computed(() => Math.ceil(pastBookings.value.length / itemsPerPage));
const totalCancelledPages = computed(() => Math.ceil(cancelledBookings.value.length / itemsPerPage));

const paginatedUpcomingBookings = computed(() => {
  const start = (upcomingCurrentPage.value - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  return upcomingBookings.value.slice(start, end);
});

const paginatedOngoingBookings = computed(() => {
  const start = (ongoingCurrentPage.value - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  return ongoingBookings.value.slice(start, end);
});

const paginatedPastBookings = computed(() => {
  const start = (pastCurrentPage.value - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  return pastBookings.value.slice(start, end);
});

const paginatedCancelledBookings = computed(() => {
...
const filterRoomTypeImage = computed(() => {
  return (roomTypeId) => {
    const room = roomType.roomtypes.find(rt => rt.id === roomTypeId);
    return room ? getImageUrl(room.image) : 'https://via.placeholder.com/300x200.png?text=No+Image';
  };
});

onMounted(async () => {
  await bookings.bookingHistory();
  await roomType.getAllRoomType();
});
